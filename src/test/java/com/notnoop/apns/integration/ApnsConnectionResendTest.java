...
public class ApnsConnectionResendTest {
	...
	private static ApnsNotification NOTIFICATION_0 = buildNotification(0);
	private static ApnsNotification NOTIFICATION_1 = buildNotification(1);
	private static ApnsNotification NOTIFICATION_2 = buildNotification(2);
	...
	public void setUp() {
		if (apnsSim == null) {
			apnsSim = new ApnsSimulatorWithVerification(FixedCertificates
	...
	/*
	 * Test when we submit 3 messages to APNS 0, 1, 2. 0 is an error but we
	 * don't see the error response back until 1,2 have already been submitted.
	 * Then at this point the network connection to APNS cannot be made, so that
	 * when retrying the submissions we have to notify the client that delivery
	 * failed for 1 and 2.
	 */
	@Test
	public void testGivenFailedSubmissionDueToErrorThenApnsDownWithNotificationsInBufferEnsureClientNotified()
		...
		apnsSim.when(NOTIFICATION_2).thenRespond(
				ApnsResponse.returnErrorAndShutdown(deliveryError,
						NOTIFICATION_0));
		...
		final List<MessageSentFailedRecord> failed = delegateRecorder
				.getFailed();
		assertEquals(3, failed.size());
		// first is failed delivery due to payload size
		failed.get(0).assertRecord(NOTIFICATION_0,
				new ApnsDeliveryErrorException(deliveryError));
	...
	private void assertNetworkIoExForRedelivery(ApnsNotification notification,
			MessageSentFailedRecord failed) {
	...
	private ApnsService build(ApnsDelegate delegate) {
		return APNS
				...
				.withGatewayDestination(LOCALHOST,
						apnsSim.getEffectiveGatewayPort())
				.withFeedbackDestination(LOCALHOST,
						apnsSim.getEffectiveFeedbackPort())
				.withDelegate(delegate).build();
	}

	private static ApnsNotification buildNotification(int id) {
		final String deviceToken = ApnsSimulatorWithVerification
				.deviceTokenForId(id);
		return new ApnsNotification(id, 1, deviceToken, "{\"aps\":{}}",
